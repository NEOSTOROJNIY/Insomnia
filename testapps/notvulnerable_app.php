			...
			<div class="span12 text-center">
				<?php 
					if(isset($_GET['username'])) { 
						echo "<hr><h5>Компания Blizzard приветствует посетителя<br><b><font color=\"red\">" . stripslashes(htmlspecialchars($_GET['username'])) . "</font></b>!</h5><hr>"; } ?>
			</div>
		</div>
		...
		<div class="row">
			...
			if( !isset($_GET['id']) )
				...
				if(!$dbhandle)
					...
					$res = mysql_query("SELECT name,description FROM games WHERE id={$id}");
					if($res) {
