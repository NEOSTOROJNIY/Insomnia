<?php
class Insomnia_Workflow_Research_XSS_Inject_DataProtocol
	extends Insomnia_Workflow_Research_XSS_XSSInjectTechnique
{
	public static $techniqueName = "Data Protocol";
	public static $techniqueID = 10;
	...
	public function execute() {
		...
		$requestHandler = new Insomnia_Common_Request_XSSRequestHandler();
			...
			$infectedParamValue = $this->dataProtocolVector;
			...
			$infectedReplyXSSVulnerable = $this->checkForXSSFacts(
				$infectedReplySet->getBody(), $this->dataProtocolVector);
			// checking of original reply.
			$originalReplyXSSVulnerable = $this->checkForXSSFacts(
				$clearReplySet->getBody(), $this->dataProtocolVector);
			...
			if($infectedReplyXSSVulnerable || $originalReplyXSSVulnerable) {
				...
				$XSSType = ( $this->checkForXSSFacts($clearReplySet->getBody(), $this->dataProtocolVector) )
				...
				if($XSSType === Insomnia_Workflow_Research_XSS_XSSCheckTechnique::XSS_UNKNOWN) {
					$XSSType = ( $this->checkForXSSFacts($nonInfectedReplySet->getBody(), $this->dataProtocolVector) )
						? Insomnia_Workflow_Research_XSS_XSSCheckTechnique::XSS_STORED
				...
				$techniqueResults[] = new Insomnia_Common_Data_XSSInjectTechResult(
					...
					true,
					$XSSType,
					array( $this->dataProtocolVector ),
					'Possibly can insert a code wich encrypted to "base64"-form and that will works under "data" protocol. Code . Example: <script src=data:;base64,YWxlcnQoJ1hTUycpOw==></script>'
				);
